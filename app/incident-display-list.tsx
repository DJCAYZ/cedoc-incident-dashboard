...
import { IncidentDisplay, Incident as DisplayIncident } from "./incident-display";
...
export function IncidentDisplayList({ status }: { status: 'active' | 'resolved' }) {
    ...
    return (
        <div className="flex flex-col gap-6 overflow-y-auto h-full pr-2 pb-10 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            {query.data?.length === 0 && (
            ...
            {query.data?.map((incident, index) => (
                <IncidentDisplay key={incident.id || index} incident={incident as DisplayIncident} />
            ))}
        </div>
    )
}