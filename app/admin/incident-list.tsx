            ...
            <div className="flex flex-col gap-3 mb-4 shrink-0 bg-slate-950/30 p-3 rounded-xl border border-slate-800/50">
                ...
                <div className="flex gap-2">
                    <div className="flex-1 min-w-0 flex flex-col gap-1">
                        <label className="text-[9px] uppercase font-bold text-slate-500 flex items-center gap-1 truncate" title="Type"><Filter size={10}/> Type</label>
                        <select 
                            value={filterType} 
                            onChange={e => setFilterType(e.target.value)}
                            className="w-full bg-slate-900 border border-slate-700 text-xs text-slate-300 rounded-md p-1.5 focus:ring-1 focus:ring-blue-500 outline-none truncate"
                        >
                            {uniqueTypes.map(t => <option key={t} value={t}>{t}</option>)}
                        </select>
                    </div>
                    <div className="flex-1 min-w-0 flex flex-col gap-1">
                        <label className="text-[9px] uppercase font-bold text-slate-500 flex items-center gap-1 truncate" title="Severity"><Filter size={10}/> Severity</label>
                        <select 
                    ...
                    <div className="flex-[1.5] min-w-0 flex flex-col gap-1">
                        <label className="text-[9px] uppercase font-bold text-slate-500 flex items-center gap-1 truncate" title="Sort By"><ArrowUpDown size={10}/> Sort By</label>
                        <select 
                            value={sortBy} 
                            onChange={e => setSortBy(e.target.value)}
                            className="w-full bg-slate-900 border border-slate-700 text-xs text-slate-300 rounded-md p-1.5 focus:ring-1 focus:ring-blue-500 outline-none truncate"
                        >
                            <option value="Time (Newest)">Newest</option>
                            <option value="Time (Oldest)">Oldest</option>
                            ...
                            <div key={incident.id} className="border-2 border-blue-500 bg-slate-900 shadow-xl shadow-blue-500/10 rounded-xl p-4 flex flex-col gap-3.5 animate-in fade-in zoom-in-95 duration-150">
                                ...
                                <div className="flex gap-2.5 mt-1 border-t border-slate-800/85 pt-3">
                                    <Button 
                                        ...
                                        disabled={isSaving} 
                                        className="bg-blue-600 hover:bg-blue-500 text-white flex-[1.5] py-2 h-auto text-xs font-bold rounded-lg flex items-center justify-center gap-1.5 cursor-pointer transition-all duration-200 hover:scale-[1.02] active:scale-[0.98]"
                                    >
                                        <Save size={12} />
                                        <span>{isSaving ? "Save" : "Save"}</span>
                                    </Button>
                                    <Button 
                                        onClick={() => { handleCancelEdit(); router.push(`?edit=${incident.id}`); }} 
                                        variant="outline" 
                                        className="border-orange-500/30 text-orange-400 hover:text-white hover:bg-orange-600/20 flex-1 py-2 h-auto text-[10px] font-bold rounded-lg cursor-pointer px-1"
                                    >
                                        Full Edit
                                    </Button>
                                    <Button 
                                        ...
                                        variant="outline" 
                                        className="border-slate-800 text-slate-400 hover:text-white hover:bg-slate-800 flex-1 py-2 h-auto text-xs font-bold rounded-lg cursor-pointer px-1"
                                    >
                                        Cancel
                                    </Button>
                                </div>
